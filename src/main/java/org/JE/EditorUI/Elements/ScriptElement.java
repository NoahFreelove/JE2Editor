...
import org.JE.ScriptElement.RemoveScriptButton;
import org.JE.ScriptElement.UIPairs.*;
...
public class ScriptElement extends UIElement {
    ...
    private FieldUIPair getType(Field field){
        ...
        if(fieldValue instanceof Vector3f vec3){
            return new Vec3Field(field, vec3, ref, field.getName());
        }
        else if(fieldValue instanceof Vector2f vec2){
            return new Vec2Field(field, vec2, ref, field.getName());
        }
        else if(fieldValue instanceof String str){
            return new StringField(field, str, ref, field.getName());
        }

        else if(fieldValue instanceof Integer i){ return new FieldUIPair(field, FieldType.INT, ref); }
        else if(fieldValue instanceof Float f){ return new FloatField(field,ref, field.getName(), Float.MIN_VALUE, f, Float.MAX_VALUE, 0.1f, 0.1f);}
        else if(fieldValue instanceof Double d){return new FieldUIPair(field, FieldType.DOUBLE, ref);}
